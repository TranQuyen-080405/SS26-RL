...
| Nút | Ghi file |
|-----|----------|
| **Apply → PC + Robot_embbed** | `RL_lib/reward_config.py` (logic train) + `Robot_embbed/modules/logics/reward_config.py` (chỉ hằng số) |
| **Copy export** | Clipboard: state preset + snippet reward |

Train tab import `compute_reward` từ `RL_lib/reward_config.py` — Apply có hiệu lực ngay sau reload trainer. ESP32 infer **không** gọi `compute_reward`; file robot chỉ cần hằng nếu firmware tham chiếu sau này.
