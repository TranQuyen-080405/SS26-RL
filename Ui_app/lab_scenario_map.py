"""Map 37×5 tương tác — đặt robot, goal, CP, tường."""
...
class LabScenarioMap5:
    def __init__(self, parent, world, on_change=None):
        ...
        self.frame = ttk.LabelFrame(parent, text="Kịch bản 37×5", padding=4)
    ...
    def _build_result_panel(self):
        ...
        self._formula_lbl.pack(fill=tk.X, pady=(0, 6))
        ...
        self._reward_box.bind("<Configure>", self._on_reward_configure)

    def _on_reward_configure(self, event):
        w = max(280, event.width - 28)
        self._formula_lbl.config(wraplength=w)
    ...
    def set_result_display(
        ...
        if has_action:
            ...
            self._action_lbl.pack(fill=tk.X, pady=(0, 4))
            self._formula_lbl.config(text="Công thức:\n" + self._format_formula(formula))
            self._formula_lbl.pack(fill=tk.X, pady=(0, 6))
